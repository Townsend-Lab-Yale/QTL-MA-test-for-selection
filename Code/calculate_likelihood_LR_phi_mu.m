...
function ltable = calculate_likelihood_LR_phi_mu(C, Q, E, thetaL, thetaR,phi, mu)
...
for j = 1: length(C)
    ...
    for i = 1:length(Q)
        ...
        muts = draw_muts(c, thetaL, thetaR, phi, mu);
        ...
        while go
            ...
            muts2 = muts + draw_muts(c, thetaL, thetaR,phi, mu);
...
function s = draw_muts(c, thetaL, thetaR, phi, mu)
delta = .5;
pdf = @(x) probfix2(c,x, thetaL,thetaR, phi, mu);
...
% text).
function prob = probfix2(c,x, thetaL, thetaR, phi, mu)
n =  2.3*10^6;
if (x<mu)
