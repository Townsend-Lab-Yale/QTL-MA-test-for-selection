...
% 
function ltable = calculate_likelihood_phi(C, Q, E,theta, phi)
...
for j = 1: length(C)
    ...
    for i = 1:length(Q)
        ...
        while go
            ...
            muts2 = muts + draw_muts(c, theta, phi);
...
function s = draw_muts(c, theta, phi)
delta = .5;
pdf = @(x) probfix2(c,x, theta, phi);
...
% text).
function prob = probfix2(c,x, theta, phi)
n =  2.3*10^6;
probability1 = exppdf(abs(x), theta);
