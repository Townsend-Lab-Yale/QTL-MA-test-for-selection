...
% estimates for the submodal and supermodal exponential decay parameters 
% and the parameter contorlling the submodal and supermodal probability
% masses of the distribution of spontaneous mutations and the mutation rate
% (u).
% 
% INPUT:
...
% thetaL0, thetaR0, and phi0 are initial guesses for thetaL and thetaR, and
% phi
% thetaLstep0, thetaRstep0, phistep0, and ustep0 are initial step sizes 
% for the algorithm
% maxiter is the number of iterations to run the hill-climbing algorithm
% 
% OUTPUT:
% thetaL is the submodal exponential decay parameter
% thetaR is the supermodal exponential decay parameter
...
% 
function [thetaL, thetaR, u, phi, likelihood] = calculate_LR_phi(changes, gens, u0, ustep0, thetaL0,thetaR0, phi0, thetaLstep0,thetaRstep0,phistep0, maxiter)

% Initialize thetaL, thetaR, phi, u and the step sizes
thetaL = thetaL0;
thetaR = thetaR0;
phi = phi0;
u = u0;
...
% phi, and u
likelihood = mutefflikelihood(thetaL,thetaR, phi, changes, gens, u, cap);
...
while i < maxiter;
  ...
  newlikelihood = mutefflikelihood(newthetaL,newthetaR, newphi, changes, gens, u, cap);
...
function [l1, l2 ] = mutefflikelihood(thetaL, thetaR, phi, changes, gens, u, cap)
l1 = 1;
l2 = 1; 
for i = 1:length(changes)
    m = muteffdist(changes(i), u*gens(i), thetaL, thetaR, phi, cap);
    l1 = l1*m(1);
    l2 = l2*m(1);
end
end
...
function [val] = muteffdist(x, lam, thetaL, thetaR , phi, cap)
Vals = zeros(1,cap);
for k = 1:cap
