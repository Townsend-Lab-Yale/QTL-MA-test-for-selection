...
function ltable = calculate_likelihood_LR_phi(C, Q, E, thetaL, thetaR,phi)
...
for j = 1: length(C)
    ...
    for i = 1:length(Q)
        ...
        while go
            ...
            muts2 = muts + draw_muts(c, thetaL, thetaR,phi);
...
% text).
function prob = probfix2(c,x, thetaL, thetaR, phi)
n = 2.3*10^6;
if (x<0)
    probability1 = exppdf(abs(x), thetaL);
else
    probability1 = exppdf(x, thetaR);
end
