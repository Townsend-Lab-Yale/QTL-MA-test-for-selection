...
% mu is a scalar, the estimated displacement parameter
% 
% OUTPUT:
...
function ltable = calculate_likelihood_LR_mu(C, Q, E, thetaL, thetaR,mu)
...
for j = 1: length(C)
    ...
    for i = 1:length(Q)
        ...
        muts = draw_muts(c, thetaL, thetaR, mu);
        ...
        while go
            ...
            muts2 = muts + draw_muts(c, thetaL, thetaR, mu);
...
% This function generates 1.0*10^6 samples from the distribution of
% mutation effects given strength of selection c by markov sampling.
function s = draw_muts(c, thetaL, thetaR, mu)
delta = .5;
pdf = @(x) probfix2(c,x, thetaL,thetaR, mu);
...
% text).
function prob = probfix2(c,x, thetaL, thetaR, mu)
n =  2.3*10^6;
if (x<mu)
    probability1 = exppdf(abs(x-mu), thetaL);
else
    probability1 = exppdf(x-mu, thetaR);
end
