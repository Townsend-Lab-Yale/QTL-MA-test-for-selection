...
function ltable = calculate_likelihood_mu(C, Q, E, theta, mu)
...
for j = 1: length(C)
    ...
    for i = 1:length(Q)
        ...
        while go
            ...
            muts2 = muts + draw_muts(c, theta, mu);
...
function s = draw_muts(c, theta, mu)
delta = .5;
pdf = @(x) probfix2(c,x, theta, mu);
...
% text).
function prob = probfix2(c,x, theta,mu)
n =  3*10^6;
